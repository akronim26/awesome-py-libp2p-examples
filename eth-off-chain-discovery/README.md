...
> [!IMPORTANT]
> Before deploying, you must create a `deploy_output.json` file in the project root with the following content:
> ```json
> {}
> ```
...
```bash
cd contracts
forge script script/DeployServiceRegistry.s.sol --rpc-url $RPC_URL --broadcast --private-key $PRIVATE_KEY
```
...
### 4. Run Publisher from `eth-off-chain-discovery`
...
A `keys/` directory is automatically created and updated with:
